#!/usr/bin/env python3
"""
Root Android App Optimization TUI
Interactive tool to optimize Android apps using root access from a Python script.
...
# Set up logging BEFORE importing Textual
import logging

# Clear log file at startup
log_file = 'root_optimizer.log'
try:
...
try:
    ...
    print("Install with: pip install textual rich pexpect", file=sys.stderr)
    sys.exit(1)
...
class RootWrapper:
    """Wrapper class to handle command execution via sudo using subprocess."""

    def __init__(self, timeout: int = 30):
        self.timeout = timeout
        self.connected = False
        self.use_termux = self._detect_termux()
    ...
    def connect(self) -> bool:
        """Initialize connection for command execution."""
        try:
            logging.info(f"Initializing connection (Termux: {self.use_termux})...")

            # For Termux, we just verify we can run commands
            # For regular systems, test sudo
            if self.use_termux:
                # In Termux, check both pm and su access
                result = subprocess.run(
                    ['sh', '-c', 'pm list packages -3 | wc -l'],
                    capture_output=True,
                    timeout=5,
                    text=True
                )
                if result.returncode == 0:
                    logging.info("Successfully verified command execution in Termux")
                    self.connected = True
                    return True
                else:
                    logging.error(f"Failed to verify command execution: {result.stderr}")
                    return False
            else:
                # Test sudo with non-interactive mode
                result = subprocess.run(
                    ['sudo', '-n', 'test'],
                    capture_output=True,
                    timeout=5,
                ...
                # Even if test fails, -n flag works (means no password)
                self.connected = True
                logging.info("Successfully verified sudo (no password required)")
                return True

        except Exception as e:
            logging.error(f"Error during connect: {str(e)}")
    ...
    def execute_command(self, command: str) -> str:
        ...
        try:
            ...
            if self.use_termux:
                # In Termux, we need special handling
                needs_root_context = any(x in command for x in ['cmd package compile', 'dumpsys', 'reboot'])

                if needs_root_context:
                    # Use su for commands that need root/shell context
                    # su runs the command with proper Android framework context
                    if not command.startswith('su -c'):
                        # Use shlex.quote for proper shell escaping
                        quoted_cmd = shlex.quote(command)
                        command = f"su -c {quoted_cmd}"
                        logging.debug(f"Wrapped with su: {command}")
                ...
                # On regular system, use sudo -n (non-interactive)
                result = subprocess.run(
                    ['sudo', '-n', 'sh', '-c', command],
                    capture_output=True,
                    timeout=self.timeout,
            ...
            if result.returncode == 0:
                ...
                logging.debug(f"Command result: {output[:200]}...")
                return output
            else:
                # If stdout is empty but stderr has content, return stderr
                if result.stdout.strip():
                    output = result.stdout.strip()
                    logging.debug(f"Command succeeded with output: {output[:200]}...")
                    return output

                error_msg = result.stderr.strip() or "Command failed with no error message"
                logging.warning(f"Command failed (return code {result.returncode}): {error_msg}")
                return f"Error: {error_msg}"

        except subprocess.TimeoutExpired:
            msg = "Command timed out"
            logging.warning(f"{msg}: {command}")
            return msg
        except Exception as e:
    ...
    def close(self):
        """Close connection (no-op for subprocess)."""
        self.connected = False
...
class OptimizationProgressScreen(Screen):
    ...
    def __init__(self, apps: List[AppInfo], profile: str, root_wrapper: RootWrapper):
        super().__init__()
        self.apps = apps
        self.profile = profile
...
"""
    ...
    def optimize(self) -> None:
        ...
        for idx, app in enumerate(self.apps):
            ...
            result = self.root_wrapper.execute_command(command)
    ...
    def show_summary(self) -> None:
        ...
        self.app.pop_screen()
        self.app.push_screen(
            SummaryScreen(self.success_count, self.failed_count, self.root_wrapper, self.app)
        )
...
class SummaryScreen(Screen):
    ...
    def __init__(self, success_count: int, failed_count: int, root_wrapper: RootWrapper, app: 'OptimizerApp'):
        super().__init__()
...
"""
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "continue_btn":
            ...
            self.root_wrapper.execute_command("reboot")
            self.parent_app.exit()
        elif event.button.id == "exit_btn":
            self.parent_app.exit()


class RootOptimizerApp:
    """Main Textual application for Root app optimizer."""
    ...
    def run(self) -> None:
        ...
        class OptimizerApp(App):
            CSS = RootOptimizerApp.CSS
            TITLE = "Root App Optimizer"

            def __init__(self):
                super().__init__()
                self.optimizer = optimizer
                self.selected_apps: List[AppInfo] = []
                self.root_wrapper = optimizer.root_wrapper
            ...
            def on_mount(self) -> None:
                ...
                try:
                    ...
                    # Connect to root
                    logging.info("on_mount: Attempting sudo connection")
                    if not self.root_wrapper.connect():
                        error_msg = "Failed to connect with sudo. Check root_optimizer.log for details."
                        logging.error(f"on_mount: {error_msg}")
                        self.title = error_msg
                        self.call_later(self.exit)
                        return

                    # Fetch apps
                    logging.info("on_mount: Fetching apps")
                    self.fetch_apps_and_show_selection()
                except Exception as e:
                    logging.error(f"on_mount error: {str(e)}")
                    logging.error(traceback.format_exc())
                    self.title = f"Initialization error: {str(e)}"
                    self.call_later(self.exit)

            def fetch_apps_and_show_selection(self) -> None:
                """Fetch apps and show selection screen."""
                try:
                    logging.info("fetch_apps_and_show_selection: Starting")

                    # Get list of user-installed packages
                    logging.debug("Fetching package list...")
                    packages_output = self.root_wrapper.execute_command(
                        "pm list packages -3 | sed 's/package://'"
                    )

                    logging.debug(f"Packages output length: {len(packages_output) if packages_output else 0}")
                    if not packages_output or "Error" in packages_output:
                        error_msg = f"Error fetching packages: {packages_output}"
                    ...
                    logging.debug("Fetching dexopt status...")
                    dexopt_output = self.root_wrapper.execute_command("dumpsys package dexopt")
                    if not dexopt_output or "Error" in dexopt_output:
                        logging.warning("Could not fetch dexopt status, using empty")
                        dexopt_output = ""

                    # Parse and create AppInfo objects
                    logging.debug("Parsing app info...")
                    self.optimizer.apps = []
                    ...
                    logging.info(f"Successfully loaded {len(self.optimizer.apps)} apps")

                    # Show app selection screen
                    self.push_screen(AppSelectionScreen(self.optimizer.apps))
                except Exception as e:
                    error_msg = f"Error in fetch_apps_and_show_selection: {str(e)}"
                    logging.error(error_msg)
                    logging.error(traceback.format_exc())
                    self.title = error_msg
...
def main():
    ...
    try:
        logging.info("=" * 80)
        logging.info("Root App Optimizer started")
        logging.info(f"Working directory: {os.getcwd()}")
        logging.info(f"Python version: {sys.version}")
        logging.info("=" * 80)

        optimizer = RootOptimizerApp()
        optimizer.run()
...
def run_diagnostics():
    """Run diagnostic checks to help debug issues."""
    print("=" * 80)
    print("ROOT OPTIMIZER - DIAGNOSTIC MODE")
    print("=" * 80)
    ...
    required_packages = ['pexpect', 'textual', 'rich']
    ...
    # Try to connect with sudo
    print("\n--- Testing command execution ---")
    wrapper = RootWrapper(timeout=10)

    # Check environment
    is_termux = os.path.exists('/data/data/com.termux')
    print(f"Environment: {'Termux' if is_termux else 'Regular system'}")

    if wrapper.connect():
        print("✓ Successfully connected")

        # Try to run pm list
        print("\n--- Testing pm list packages ---")
        result = wrapper.execute_command("pm list packages -3 | head -5")
        if result and "Error" not in result:
            print(f"✓ pm command works. First 5 packages:")
            for line in result.split('\n')[:5]:
                if line.strip():
                    print(f"  • {line.strip()}")
        else:
            print(f"✗ Error running pm command: {result}")

        # Try dumpsys
        print("\n--- Testing dumpsys package dexopt ---")
        result = wrapper.execute_command("dumpsys package dexopt | head -10")
        if result and "Error" not in result:
            print(f"✓ dumpsys command works. Output length: {len(result)} chars")
            print(f"First line: {result.split(chr(10))[0] if result else 'N/A'}")
        else:
            print(f"✗ Error running dumpsys command")
            if result:
                print(f"  Error: {result[:200]}")

        # Try a compile command (with su in Termux)
        if is_termux:
            print("\n--- Testing cmd package compile (dry-run) ---")
            result = wrapper.execute_command("cmd package compile -m verify -f com.android.systemui")
            if result and "Error" not in result and "SecurityException" not in result:
                print(f"✓ cmd package compile works")
            else:
                print(f"✗ Error running cmd package compile")
                if "SecurityException" in result:
                    print("  Issue: ART service needs proper su context")
                if result:
        ...
        if not is_termux:
            print("  Make sure you have passwordless sudo configured")
        else:
            print("  Make sure pm and dumpsys commands are available")

    print("\n" + "=" * 80)
    print("Diagnostic complete. Check root_optimizer.log for detailed logs.")
    print("=" * 80)
